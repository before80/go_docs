...
> 原文：[https://beego.wiki/docs/mvc/view/view/]({{< ref "/beego/mvcIntroduction/views/templateParsing" >}})
...
​	Beego 使用 Go 内置的包 `html/template` 作为模板解析器。在启动时，它会将模板编译并缓存到一个映射中，以便高效渲染。
...
​	Beego 的默认模板目录是 `views` 。模板文件可以放入此目录，Beego 将自动解析并缓存它们。但是，如果启用了开发模式，Beego 每次都会解析模板，而不会缓存。Beego 只能有一个可以自定义的模板目录：
...
​	这将在该目录中解析并缓存模板文件，并允许您通过在控制器上设置 ViewPath 来使用它们：
...
​	将 ViewPath 设置为先前未使用 AddViewPath() 注册的目录将失败，并显示“未知视图路径”
...
​	您无需手动渲染和输出模板。Beego 将在完成方法后自动调用 Render。如果您不需要，可以在配置文件或 `main.go` 中禁用自动渲染。
...
​	Go 使用 `{{` 和 `}}` 作为默认模板标记。如果这些标记与其他模板标记（如 AngularJS 中的标记）冲突，我们可以使用其他标记。为此，在配置文件中：
...
​	模板从 Controller 中的 `this.Data` 获取数据。因此，例如，如果您需要在模板中使用 `{{.Content}}` ，则需要先在 Controller 中分配它：
...
>
> ​	从版本 1.6 开始：this.TplNames 为 this.TplName
...
​	Beego 使用 Go 内置的模板引擎，因此语法与 Go 相同。要详细了解模板，请参阅模板。
...
​	您可以在控制器中设置模板名称，Beego 将在 viewpath 下找到模板文件并自动呈现它。在下面的配置中，Beego 将在 admin 下找到 add.tpl 并呈现它。
...
​	Beego 默认支持 `.tpl` 和 `.html` 文件扩展名。如果您使用其他扩展名，则必须首先在配置中设置它：
...
​	如果在启用 `autorender` 时未在控制器中设置 `TplName` ，则 Beego 将默认使用 `TplName` ，如下所示：
...
​	它是控制器名称 + “/” + 请求方法名称 + “.” + 模板扩展名。因此，如果控制器名称为 `AddController` ，请求方法为 `POST` ，默认模板扩展名为 `tpl` ，则 Beego 将呈现 `/viewpath/addcontroller/post.tpl` 模板文件。
...
​	Beego 支持布局设计。例如，如果在您的应用程序中，主导航和页脚不会改变，只有内容部分不同，您可以使用这样的布局：
...
​	Beego 将解析名为 `TplName` 的文件，并将其分配给 `LayoutContent` ，然后渲染 `layout.html` 。

Beego will cache all the template files. You can also implement a layout this way:

​	Beego 将缓存所有模板文件。您还可以通过这种方式实现布局：
...
​	 `LayoutContent` 有点复杂，因为它可以包含 Javascript 和 CSS。由于在大多数情况下，仅有一个 `LayoutContent` 是不够的，因此在 `Controller` 中有一个称为 `LayoutSections` 的属性。它允许我们在 `Layout` 页面中设置多个 `section` ，并且每个 `section` 都可以包含其自己的子模板页面。
...
​	使用 `block` 操作代替 `template` 允许我们拥有默认块内容，并跳过我们不需要的块（例如，如果我们不需要 `add.tpl` 模板中的 css 块 - 我们将不会定义它，并且不会引发错误）
...
- StructTag definition uses `form` as tag. It uses the same tags as [Parse Form]({{< ref "/beego/mvcIntroduction/controllers/requestParameters#parse-to-struct" >}}). There are three optional params separated by ‘,’:

  ​	StructTag 定义使用 `form` 作为标签。它使用与解析表单相同的标签。有三个可选参数，用“,”分隔：
...
- If the `form` tag only has one value, it is the `name` attribute of the form field. Except last value can be ignore all the other place must be separated by ‘,’. E.g.: `form:",,username:"`
  ...
  ​	如果 `form` 标签只有一个值，则它是表单字段的 `name` 属性。除了最后一个值可以忽略之外，其他所有地方都必须用“,”分隔。例如： `form:",,username:"`
