...
# 4 – The Application Program Interface 4 – 应用程序接口
...
### 4.1.2 – Valid and Acceptable Indices 4.1.2 – 有效和可接受的索引
...
### 4.1.3 – Pointers to strings 4.1.3 – 指向字符串的指针
...
## 4.4 – Error Handling in C 4.4 – C 中的错误处理
...
## 4.5 – Handling Yields in C 4.5 – 在 C 中处理 Yield
...
## 4.6 – Functions and Types 4.6 – 函数和类型
...
## 4.7 – The Debug Interface 4.7 – 调试接口
...
Gets information about a local variable or a temporary value of a given activation record or a given function. 重试 错误原因

In the first case, the parameter `ar` must be a valid activation record that was filled by a previous call to [`lua_getstack`](https://www.lua.org/manual/5.4/manual.html#lua_getstack) or given as argument to a hook (see [`lua_Hook`](https://www.lua.org/manual/5.4/manual.html#lua_Hook)). The index `n` selects which local variable to inspect; see [`debug.getlocal`](https://www.lua.org/manual/5.4/manual.html#pdf-debug.getlocal) for details about variable indices and names. 重试 错误原因

[`lua_getlocal`](https://www.lua.org/manual/5.4/manual.html#lua_getlocal) pushes the variable's value onto the stack and returns its name. 重试 错误原因
