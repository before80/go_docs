...
包tabwriter实现了一个写入过滤器（tabwriter.Writer），它将输入中的制表符列转换为正确对齐的文本。
...
NewWriter分配并初始化一个新的tabwriter.Writer。参数与Init函数相同。
...
Flush应在最后一次调用Write后调用，以确保将Writer中缓冲的任何数据写入输出。对于格式化目的，尾部的任何不完整的转义序列都被视为完整。
...
Write将buf写入写入器b。仅返回在写入底层输出流时遇到的错误。