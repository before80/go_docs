...
Since Go 1.13, the go command by default downloads and authenticates modules using the Go module mirror and Go checksum database. See [proxy.golang.org/privacy](https://proxy.golang.org/privacy) for privacy information about these services and the [go command documentation](https://golang.org/ref/mod#private-module-privacy) for configuration details including how to disable the use of these servers or use different ones. If you depend on non-public modules, see the [documentation for configuring your environment](https://pkg.go.dev/cmd/go#hdr-Configuration_for_downloading_non_public_code).

从Go 1.13开始，go命令默认使用Go模块镜像和Go校验数据库下载和验证模块。关于这些服务的隐私信息，请参见proxy.golang.org/privacy，关于配置细节，包括如何禁用这些服务器或使用不同的服务器，请参见go命令文档。如果您依赖非公开的模块，请参阅配置环境的文档。
...
proxy.golang.org - 一个模块镜像，实现了模块代理协议。对于下载大量模块的用户（例如用于批量静态分析），镜像支持一个非标准的头，Disable-Module-Fetch: true，指示它只返回缓存的内容。这将避免缓慢的下载，但代价是可能会错过一些很少使用的模块。
...
sum.golang.org - 一个可审计的校验数据库，将被go命令用来验证模块。在 go 命令的文档中了解更多。
...
index.golang.org - 一个索引，为proxy.golang.org提供新模块版本的信息。可以在 https://index.golang.org/index 上查看该信息。馈送是以新行分隔的JSON格式提供的，提供了模块的路径（Path），模块的版本（Version），以及它第一次被proxy.golang.org缓存的时间（Timestamp）。该列表按时间顺序排序。有两个可选参数。
...
- since"：在返回的列表中，模块版本允许的最老的时间戳（RFC3339格式）。默认是时间的开始，例如：https://index.golang.org/index?since=2019-04-10T19:08:52.997264Z
- 'limit': the maximum length of the returned list. Default = 2000, Max = 2000, e.g. https://index.golang.org/index?limit=10
- 'limit'：返回列表的最大长度。默认=2000，最大=2000，例如：https://index.golang.org/index?limit=10
...
如果您使用该索引从模块镜像中下载许多模块，您要设置Disable-Module-Fetch头，如上所述。
...
这些服务已经可以在生产中使用了。如果您发现了这些问题，请提交问题，标题前缀为 "proxy.golang.org:"（或 "index.golang.org:"，或 "sum.golang.org:"）。
...
这些服务只能访问公开的源代码。如果您依赖私有模块，请将GOPRIVATE设置为涵盖它们的glob模式。更多细节请参见 go 命令文档中的非公开模块的模块配置。

To opt-out of this module mirror, you can turn it off by setting `GOPROXY=direct`

要退出这个模块镜像，您可以通过设置GOPROXY=direct来关闭它。
...
我向版本库提交了一个新的修改（或发布了一个新的版本），为什么当我运行 go get -u 或 go list -m --versions 时，它没有显示出来？
...
为了改善我们服务的缓存和服务延迟，新版本可能不会马上显示出来。如果您想让新的代码立即出现在镜像中，那么首先要确保在底层源码库中有一个语义上的版本标签。然后通过 go get module@version 明确请求该版本。新版本应该在一分钟内可用。请注意，如果有人在标签被推送之前请求该版本，可能需要长达30分钟的时间让镜像的缓存过期，关于该版本的新数据才会变得可用。如果30分钟后该版本仍然不可用，请提交一个问题。

### I removed a bad release from my repository but it still appears in the mirror, what should I do?

我从我的版本库中删除了一个坏的版本，但它仍然出现在镜像中，我应该怎么做？
...
只要有可能，镜像的目的是缓存内容，以避免破坏依赖您的软件包的人的构建，所以这个坏版本可能仍然在镜像中可用，即使它在原点不可用。如果您删除了您的整个版本库，同样的情况也适用。我们建议创建一个新的版本并鼓励人们使用这个版本。
...
如果您想从 go 命令以及 pkg.go.dev 中隐藏模块的版本，您应该撤回它们。撤回模块版本需要在go.mod文件中加入retract指令并发布新的版本。详情请参见Go博客文章《Go 1.16中的新模块变化》和模块参考。
...
代理和校验数据库协议只向远程服务器发送模块的路径和版本。如果您请求一个私有模块，镜像将尝试下载它，就像任何Go用户一样，并以同样的方式失败。关于失败的请求的信息不会在任何地方公布。请求的唯一痕迹将出现在内部日志中，这受隐私政策的约束。
...
proxy.golang.org不会永远保存所有模块。这有很多原因，但其中一个原因是如果proxy.golang.org无法检测到合适的许可证。在这种情况下，只有一个临时缓存的模块副本将被提供，如果它被从原始源头移除并变得过时，则可能变得不可用。无论镜像中是否变得不可用，校验和仍将保留在校验和数据库中。
...
遵循安全政策，发送电子邮件到 security@golang.org，并在邮件的某处写上 "漏洞 "一词。