...
从Go 1.13开始，go命令默认使用Go模块镜像和Go校验数据库下载和验证模块。关于这些服务的隐私信息，请参见proxy.golang.org/privacy，关于配置细节，包括如何禁用这些服务器或使用不同的服务器，请参见go命令文档。如果你依赖非公开的模块，请参阅配置环境的文档。
...
如果你使用该索引从模块镜像中下载许多模块，你要设置Disable-Module-Fetch头，如上所述。
...
这些服务已经可以在生产中使用了。如果你发现了这些问题，请提交问题，标题前缀为 "proxy.golang.org:"（或 "index.golang.org:"，或 "sum.golang.org:"）。
...
这些服务只能访问公开的源代码。如果你依赖私有模块，请将GOPRIVATE设置为涵盖它们的glob模式。更多细节请参见 go 命令文档中的非公开模块的模块配置。

To opt-out of this module mirror, you can turn it off by setting `GOPROXY=direct`

要退出这个模块镜像，你可以通过设置GOPROXY=direct来关闭它。
...
为了改善我们服务的缓存和服务延迟，新版本可能不会马上显示出来。如果你想让新的代码立即出现在镜像中，那么首先要确保在底层源码库中有一个语义上的版本标签。然后通过 go get module@version 明确请求该版本。新版本应该在一分钟内可用。请注意，如果有人在标签被推送之前请求该版本，可能需要长达30分钟的时间让镜像的缓存过期，关于该版本的新数据才会变得可用。如果30分钟后该版本仍然不可用，请提交一个问题。
...
只要有可能，镜像的目的是缓存内容，以避免破坏依赖你的软件包的人的构建，所以这个坏版本可能仍然在镜像中可用，即使它在原点不可用。如果你删除了你的整个版本库，同样的情况也适用。我们建议创建一个新的版本并鼓励人们使用这个版本。
...
如果你想从 go 命令以及 pkg.go.dev 中隐藏模块的版本，你应该撤回它们。撤回模块版本需要在go.mod文件中加入retract指令并发布新的版本。详情请参见Go博客文章《Go 1.16中的新模块变化》和模块参考。
...
代理和校验数据库协议只向远程服务器发送模块的路径和版本。如果你请求一个私有模块，镜像将尝试下载它，就像任何Go用户一样，并以同样的方式失败。关于失败的请求的信息不会在任何地方公布。请求的唯一痕迹将出现在内部日志中，这受隐私政策的约束。
