...
​	当发布预发布版本时，你可以发布不保证稳定性或向后兼容性的发布版本，但给您的用户一个机会来试用模块并给您反馈。

​	不稳定的发布版本是指其版本号在 v0.x.x 范围内的版本。v0 版本不做任何稳定性或向后兼容性的保证。但它为你在进行 v1 及之后的稳定性承诺之前，获得反馈并完善 API 提供了一种方式。更多信息请参见 [模块版本编号](../ModuleVersionNumbering)。

​	与其他发布版本一样，你可以在向发布稳定的 v1 版本迈进的过程中递增 v0 版本号的次要版本和修订版本。例如，在发布了 v0.0.0 后，你可以发布带有第一组错误修复的 v0.0.1。
...
​	通过 v1 版本的发布，你向使用你的模块的开发者做出以下承诺：
...
**注意：** 虽然你的第一个主版本可能是一个 v0 发布，但 v0 版本不表示稳定性或向后兼容性的保证。因此，当你从 v0 升级到 v1 时，无需考虑破坏向后兼容性，因为 v0 发布被认为不稳定。
...
​	你通过在代码库中标记模块代码，并在标签中指定 v1 版本号来发布第一个稳定版本。更多信息请参见[发布模块](../PublishingAModule)。

## 发布bug修复

​	你可以发布仅包含错误修复的版本。这被称为修订版本。
...
**注意：** 你的修订版本应尽量避免将该模块自身的传递依赖项（transitive dependencies）升级超过修订版本。否则，升级到你的模块的修订版本时，可能会意外地引入对使用的传递依赖项（transitive dependency）的更大干扰性更改。（transitive 该怎么翻译？）
...
​	你可以通过在代码库中标记模块代码，并在标签中递增修订版本号来发布修订版本。更多信息请参见[发布模块](../PublishingAModule)。

## 发布非破坏性的API变更

​	你可以对模块的公共 API 进行非破坏性的变更，并将这些变更发布为*次要*版本。
...
​	你可以通过在代码库中标记模块代码，并在标签中递增次要版本号来发布次要版本。更多信息请参见[发布模块](../PublishingAModule)。

## 发布破坏性的API变更

​	你可以通过发布一个*主要*版本来引入破坏性的向后不兼容变更。

​	主要版本发布不保证向后兼容性，通常是因为它包含了对模块的公共 API 进行的变更，这些变更会破坏使用该模块先前版本的代码。

​	考虑到主要版本升级对依赖该模块的代码可能产生的破坏性影响，如果可能的话，应避免进行主要版本更新。有关主要版本更新的更多信息，请参阅[开发主要版本更新](../DevelopingAmajorVersionUpdate)。有关避免进行破坏性变更的策略，请参阅博文[保持模块的兼容性]({{< ref "/docs/GoBlog/2020/KeepingYourModulesCompatible">}})。

​	发布其他类型的版本基本上只需通过标记模块代码与版本号，而发布主要版本更新则需要更多步骤。

1. 在开始开发新的主要版本之前，在你的代码库中创建一个用于新版本源代码的位置。
...
4. 与任何新版本一样，你应该发布预发布版本以获得反馈和bug报告，然后再发布官方版本。

5. 通过在你的代码库中标记该模块代码并递增标记中的主要版本号来发布新的主要版本，例如从 v1.5.2 到 v2.0.0。
