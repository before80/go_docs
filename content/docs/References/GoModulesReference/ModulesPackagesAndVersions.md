...
​	主版本后缀允许模块的多个主版本共存于同一个构建中。这可能是由于[diamond dependency problem（钻石依赖项问题）](https://research.swtch.com/vgo-import#dependency_story)而需要的。通常，如果一个模块在两个不同的版本中被传递依赖关系所需要，则将使用更高的版本。然而，如果这两个版本不兼容，那么这两个版本都不能满足所有的客户。由于不兼容的版本必须有不同的主版本号，因此由于主版本后缀，它们也必须具有不同的模块路径。这就解决了冲突：具有不同后缀的模块被视为独立的模块，它们的包 —— 即使是相对于它们的模块根的同一子目录下的包 —— 也是不同的。
