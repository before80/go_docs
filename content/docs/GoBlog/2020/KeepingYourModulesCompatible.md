...
# Keeping Your Modules Compatible - 保持你的模块兼容
...
你的模块会随着时间的推移而发展，因为你会添加新的功能，改变行为，以及重新考虑模块的部分公共表面。正如《Go Modules: v2 and Beyond》中所讨论的，对 v1+ 模块的突破性改变必须作为主要版本升级的一部分（或通过采用新的模块路径）。
...
然而，发布一个新的主要版本对你的用户来说是困难的。他们必须找到新的版本，学习新的API，并改变他们的代码。而且有些用户可能永远不会更新，这意味着你必须永远为你的代码维护两个版本。所以，通常情况下，以一种兼容的方式改变你现有的包是更好的。
...
在这篇文章中，我们将探讨一些引入非破坏性变化的技巧。共同的主题是：添加，不要改变或删除。我们还将讨论如何从一开始就为兼容性设计你的API。
...
这个例子说明了调用兼容性对于向后兼容是不够的。事实上，你不能对函数的签名做任何向后兼容的改变。
...
如果你预计一个函数在未来可能需要更多的参数，你可以通过将可选参数作为函数签名的一部分来提前计划。最简单的方法是添加一个结构参数，就像 crypto/tls.Dial 函数那样：
...
无论哪一种都是合理的选择，以确保你的模块的公共API未来的可扩展性。
...
让我们用archive/tar包中的一个例子来说明。tar.NewReader接受io.Reader，但随着时间的推移，Go团队意识到，如果你能调用Seek，从一个文件头跳到下一个文件头会更有效率。但是，他们不能给io.Reader增加一个Seek方法：这将破坏io.Reader的所有实现者。
...
当你遇到要给一个现有的接口添加方法的情况时，你也许可以遵循这个策略。首先，用你的新方法创建一个新的接口，或者确定一个现有的接口，用新方法。接下来，确定需要支持它的相关函数，对第二个接口进行类型检查，并添加使用它的代码。
...
这种策略只有在没有新方法的旧接口仍能被支持时才有效，这限制了你的模块未来的可扩展性。
...
在可能的情况下，最好是完全避免这类问题。例如，在设计构造函数时，最好返回具体类型。与接口不同，使用具体类型的工作允许你在未来增加方法而不破坏用户，。这一特性使你的模块在未来更容易被扩展。
...
提示：如果你确实需要使用一个接口，但不打算让用户实现它，你可以添加一个未导出的方法。这可以防止在你的包之外定义的类型在没有嵌入的情况下满足你的接口，使你可以在以后添加方法而不破坏用户的实现。例如，请看 testing.TB 的 private() 函数。
...
我们在上面看到，对函数签名的任何改变都是一种破坏性改变。对于结构体来说，情况要好得多。如果你有一个导出的结构类型，你几乎总是可以添加一个字段或删除一个未导出的字段而不破坏兼容性。在添加字段时，要确保其零值是有意义的，并保留旧的行为，这样现有的不设置字段的代码就能继续工作。
...
你应该在你的结构中使用doNotCompare吗？如果你定义的结构是作为指针使用的，也就是说，它有指针方法，也许还有一个返回指针的 NewXXX 构造函数，那么添加 doNotCompare 字段可能是多余的。指针类型的用户知道该类型的每个值都是不同的：如果他们想比较两个值，他们应该比较指针。
...
如果你定义的结构打算直接作为一个值使用，就像我们的Point例子，那么你经常希望它是可比较的。在不常见的情况下，你有一个不希望被比较的值结构，那么添加一个 doNotCompare 字段将给你自由，以后可以改变这个结构而不必担心破坏比较。缺点是，该类型不能作为地图键使用。
...
在从头开始规划API时，要仔细考虑API对未来新变化的可扩展性如何。当你确实需要添加新的功能时，记住这个规则：添加，不要改变或删除，牢记例外情况--接口、函数参数和返回值不能以向后兼容的方式添加。
...
如果你需要大幅度地改变一个API，或者一个API随着更多的功能的增加而开始失去重点，那么可能是时候进行新的主要版本了。但大多数时候，做一个向后兼容的改变是很容易的，而且可以避免给你的用户带来痛苦。
