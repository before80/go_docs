...
​	除了Go 1.21对[向后兼容性的扩展承诺](https://go.dev/blog/compat)之外，Go 1.21还引入了更好的Go代码前向兼容性，这意味着Go 1.21及更高版本将更好地避免对需要更新版本的Go的代码进行错误编译。具体而言，`go.mod`文件中的`go`行现在指定了所需的最低Go工具链版本，而在以前的版本中，这只是一个大致没有强制执行的建议。
