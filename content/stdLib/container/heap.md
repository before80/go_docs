...
draft = false
+++
> 原文：[https://pkg.go.dev/container/heap@go1.24.2](https://pkg.go.dev/container/heap@go1.24.2)
...
​	heap包为任何实现`heap.Interface`的类型提供堆操作。堆是一棵树，其属性是每个节点都是其子树中的最小值节点。
...
​	heap 是实现优先级队列的一种常见方式。要建立一个优先级队列，要用(负)优先级作为Less方法的排序来实现Heap接口，因此Push增加项目，而Pop从队列中删除优先级最高的项目。实例中包括这样的实现；文件`example_pq_test.go`中有完整的源代码。
...
​	该接口类型描述了一个使用本包中的例程的类型的要求。任何实现它的类型都可以作为最小堆使用，并具有以下不变性(在调用Init函数后或数据为空或被排序后建立)：
