...
f := min  // 非法的：在非调用情况下使用 min 时，min 必须用类型实参实例化
minInt := min[int] // minInt 类型为 func(x, y int) int  <= 这属于非调用的情况
a := minInt(2, 3)          // a 的类型为 int，值为 2 
b := min[float64](2.0, 3)  // b 的类型为 float64，值为 2.0 
c := min(b, -1)            // c 的类型为 float64，值为 -1.0 
```
